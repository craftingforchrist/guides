...
Every report reaches our staff on Discord, including reports made with the in-game `/report` command, so you don't need a Discord account to report someone.

A staff member then picks it up:
* **Griefing:** they log in, inspect what the reported player did, roll the damage back if the report holds, and issue a punishment.
* **Chat offences** (such as swearing or harassment): they check the server-side chat logs and issue a punishment accordingly.
...
:::tip Want to hear back?

`/report` is the fastest way to reach staff, but it doesn't send you an update afterwards. If you'd like to hear back about what happened, open a support ticket on our [Discord](https://craftingforchrist.net/discord) with `/support create` instead. Tickets let you report a player too.
...
1. Stop replying.
2. Take a screenshot of the messages.
3. Report it using `/report` in our Discord, or open a support ticket with `/support create` and include the screenshot.
4. Block the user (see below).

You can also stop people you don't know from messaging you. Right-click (or tap and hold) our server's icon, open **Privacy Settings**, and turn off **Direct Messages**. This stops members of our server who aren't your friends from starting a DM with you.

### Blocking a User
Blocking someone stops them from sending you direct messages, and hides their messages from you in shared servers.

* **On desktop:** Right-click the user's name, then select **Block**.
* **On mobile:** Tap the user's name to open their profile, tap the **⋯** menu, then tap **Block**.

The other person isn't told that you blocked them.

### Leaving a Server
If you're in any Discord server where you don't feel safe, you can leave it at any time.

* **On desktop:** Right-click the server icon, then select **Leave Server**.
